...
{ 
	...
	for (i=0; i< TIC_TAC_TOE_COLS; i++)
	{
		for(j=0; j< TIC_TAC_TOE_ROWS; j++)
		{
			...
			{
				ttc_button=new_button(j*TTC_WBTN+TTC_XOFFSET,i*TTC_HBTN+TTC_XOFFSET,TTC_BTNXPATH,1,NULL,1);
			}
			else if (game_state[i*TIC_TAC_TOE_ROWS + j] == TTC_PLAYER_2)	
			{
				ttc_button=new_button(j*TTC_WBTN+TTC_XOFFSET,i*TTC_HBTN+TTC_XOFFSET,TTC_BTNOPATH,1,NULL,1);
			}
			else
			{
