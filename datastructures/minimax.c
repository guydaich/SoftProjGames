...
{
	int cMove = 0; 
	vertex root = build_tree(game_matrix, 1, depth,scoring_function,create_children);	// build tree
	if (root == NULL){
		exit(0);
	}
...
int get_computer_move(int * game_matrix,int depth,int (*scoring_function)(int* game_matrix),int (*create_children)(vertex parent, int player))
{
	int cMove = 0;
	vertex root = build_tree(game_matrix, -1, depth,scoring_function,create_children);
	if (root == NULL){
		exit(0);
	}
...
{
	int error;
	vertex root = make_node(0, game_matrix,
				 scoring_function(game_matrix)); 			// new root 
	if (root == NULL){
		return NULL;
	}
	error=create_children(root, -1 * player);	// children belong to opposite player
	if (error == -1){								// problem with std function
		remove_bad_subtree(root, 1);						// remove bad 
		return NULL;
	}
	error = recursive_tree_build(root, -1 * player, 1, maxDepth,create_children);	// construct tree	
	if (error == -1){
		remove_tree(root, 1);
		return NULL;
	}
...
int recursive_tree_build(vertex root, int player, int depth, int max_depth,int (*create_children)(vertex parent, int player))
{
	...
	else
	{
			...
			cur_elem != NULL; cur_elem = cur_elem->next)
		{
			...
			error=create_children(cur_node, -1 * player);			// add children
			if (error == -1){
				return error;
			}
			error=recursive_tree_build(cur_node, -1 * player, depth + 1, max_depth,create_children);
			if (error == -1){
				return error;
			}
		}
